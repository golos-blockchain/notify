...
const LOG_CLOUD = config.has('cloud_push.log')
...
async function putToCloud(account, scope, opData, timestamp) {
    try {
        ...
        for (const obj of tokens) {
            ...
            if (LOG_CLOUD) {
                console.log('Firebase push sent:', account, opData[0], token)
            }
            try {
...
module.exports = function useFirebaseApi(app) {
    ...
    router.get('/firebase', async (ctx) => {
        ctx.body = {
            ...
            debugRoutes: [
                '/firebase/debug/send/:token'
            ],
        }
    })

    router.get('/firebase/debug/send/:token', async (ctx) => {
